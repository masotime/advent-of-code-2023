...
export default () => {
    ...
    for (const brickToDisintegrate of world.bricks) {
        ...
        // console.log(`Without ${brickToDisintegrate.name}, the following bricks fell:`, bricksFallenAfterDisintegration.join(','))
        // console.log(render(disintegrated))
        if (bricksFallenAfterDisintegration.length === 0) {
            safeToDisintegrate.push(brickToDisintegrate.name)
        }
    }

    console.log(`It is safe to disintegrate ${safeToDisintegrate.join(',')} or ${safeToDisintegrate.length} of them`)


    return safeToDisintegrate.length
}
