...
const expand = (img: Image): Image => {
    ...
    const expandedImage = {
        rows: img.rows + rowsToExpand.size * (FACTOR - 1) - 1, // why?
        cols: img.cols + colsToExpand.size * (FACTOR - 1) - 1,
    ...
    for (const galaxy of img.galaxies) {
        const rowIncrement = rowsList.filter(row => galaxy.row > row).length * (FACTOR - 1)
        const colIncrement = colsList.filter(col => galaxy.col > col).length * (FACTOR - 1)
