...
export default () => {
    ...
    // use the distance map to re-construct a map of the main stuff
    // without all the distractions of junk
    const focusedMap: Map = {
        maxRows: map.maxRows,
        maxCols: map.maxCols,
        nodes: {}
    }

    for (const row in distanceMap.nodes) {
        for (const col in distanceMap.nodes[row]) {
            focusedMap.nodes[row] = focusedMap.nodes[row] || []
            focusedMap.nodes[row][col] = map.nodes[row][col]
        }
    }

    type SearchState = 'I' | 'O'
    type Encounter = string | undefined
    let count = 0
    for (let row = 0; row <= focusedMap.maxRows; row += 1) {
        let state: SearchState = 'O'
        let last: Encounter = undefined
        let lastConnectorDirection: string | undefined = undefined
        for (let col = 0; col <= focusedMap.maxCols; col += 1) {

            const encounter: Encounter = focusedMap.nodes[row]?.[col]

            if (encounter === '|') {
                state = state === 'O' ? 'I' : 'O'
            } else if (!lastConnectorDirection) {
                if (connectors.south.includes(encounter)) {
                    lastConnectorDirection = 'south'
                } else if (connectors.north.includes(encounter)) {
                    lastConnectorDirection = 'north'
                }
            } else if (lastConnectorDirection) {
                if (lastConnectorDirection === 'south' && connectors.north.includes(encounter) ||
                    lastConnectorDirection === 'north' && connectors.south.includes(encounter)) {
                    // flip
                    state = state === 'O' ? 'I' : 'O'
                    //reset
                    lastConnectorDirection = undefined

                } else if (lastConnectorDirection === 'south' && connectors.south.includes(encounter) ||
                    lastConnectorDirection === 'north' && connectors.north.includes(encounter)) {
                    //reset
                    lastConnectorDirection = undefined
                }
            }
            ...
            if (encounter === undefined) {
                focusedMap.nodes[row] = focusedMap.nodes[row] ?? []
                focusedMap.nodes[row][col] = state
                lastConnectorDirection = undefined
                if (state === 'I') {
                    count += 1
                }
            ...
            if (row === 7) {
                console.log(`${encounter ?? '.'}: last ${lastConnectorDirection}, state ${state}`)
            }
